...
function ScoreLine({ e }: { e: ScoreEvent }) {
  ...
  return (
    <div className="flex items-center justify-between border-b border-slate-800 pb-2 text-sm">
      ...
      <span className={played ? 'text-white font-semibold' : 'text-purple-300 text-xs'}>
...
export default function FootballPanel() {
  const { data, loading, error, refresh } = useLiveData(api.football, 30000);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-2xl font-bold">Real Madrid</h2>
        </div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <Card className="bg-slate-900 border-slate-800">
          <CardHeader>
            <CardTitle className="text-slate-100">Next Fixtures</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            {data?.nextFixtures?.length ? (
              data.nextFixtures.map((e) => <ScoreLine key={e.id} e={e} />)
            ) : (
              <p className="text-slate-500 text-sm">No upcoming fixtures found.</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="bg-slate-900 border-slate-800">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-slate-100">
              <ListOrdered className="w-4 h-4 text-purple-400" /> {data?.league || 'La Liga'} Table
            </CardTitle>
          </CardHeader>
          <CardContent>
            {data?.laLigaTable?.length ? (
              <table className="w-full text-xs">
                ...
                <tbody>
                  {data.laLigaTable.map((row) => (
                    <tr
                      key={row.position}
                      className={`border-t border-slate-800 ${row.team.toLowerCase().includes('real madrid') ? 'text-purple-300 font-semibold' : 'text-slate-300'}`}
                    >
              ...
              <p className="text-slate-500 text-sm">League table unavailable.</p>
            )}
          </CardContent>
        </Card>
      </div>
